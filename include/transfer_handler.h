...
public:
std::string createFile(long long size,std::string hash);//size in byte
...
int getFileContent(std::string fpath,long long begin,int size,void* content);

int removeFile(std::string fpath);

//begin starts from 0. content must point to memory area keeps larger area in reserve than which size parament declares.
int fillFileContent(std::string fpath,long long begin,int size,void* content);
